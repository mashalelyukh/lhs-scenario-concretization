...
#BO with minimum desired criticality C, before implementing the loop and re-evaluation:
    ...
    def encode(self, X_raw): # list of lists to numeric array
        ...
        for point in X_raw:
            ...
            for (name, kind, meta), raw in zip(self.dims, point):
                ...
                if kind == 'real':
                    row.append(raw)
                elif kind == 'int':  # or `kind=='int'` if you distinguish
                    # keep the actual integer
                    row.append(raw)
                else:
                    # only for enums
                    cats = meta
